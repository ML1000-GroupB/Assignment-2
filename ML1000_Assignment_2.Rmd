...
US Superstore dataset is sourced from [US uperstore dataset](https://www.kaggle.com/juhi1994/superstore) . The dataset has online orders for Superstores in U.S. from 2014-2018. Tableau community is the owner of the dataset. The dataset has 9994 records and 21 attributes.
...
boxplot(data$Sales, main="Sales Boxplot", xlab="Sales")
hist(data$Sales)
boxplot(data$Quantity, main="Quantity Boxplot", xlab="Quantity")
```
```{r }
summary(data$Sales)
...
```{r fig.cap= "From top: a) Boxplot of discount values. A strange looking box dataplot - median & 3rd quantile are the same (0.2) and not many orders have high discounts. b)Boxplot of profit. Most of the profits are outside of the box - but most of them clustered close to the box(not with so extreme values "}
par(mfrow=c(1,2))
boxplot(data$Discount, main="Discount")
boxplot(data$Profit, main="Profit")
summary(data$Discount)
summary(data$Profit)
```
```{r fig.cap="Plot of transactions involving discounts. Sales transactions mostly do not involve discounts."}
ggplot(data = data) +
...
```{r fig.cap= "Profit distribution from transactions"}
ggplot(data)+
...
For the minimum value of K, the LOF values fluctuate wildy the points in a uniform distribution for k<10, with points in a uniform distribution sometimes showing up as outliers, so recommended value of k is min 10.The minimum k-value serves as a minimum size for something to be considered a "cluster", so points outside can be outliers relative to that cluster. 
...
## Deployment
The clustering model and anomaly detection app are deployed in shiny app. The interactive applications accepts inputs from the user and display accordingly. The url for application is below  
