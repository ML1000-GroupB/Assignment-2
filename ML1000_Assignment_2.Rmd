...
```{r, fig.cap = "Histogram of time difference between order date and ship date. The time difference between order date and ship date typically takes 4 days." }
...
```{r fig.cap ="Boxplot of sales showing many outliers."}
boxplot(data$Sales)
```

```{r fig.cap= "Histogram of sales values."}
hist(data$Sales)
...
```{r fig.cap= "Boxplot of quantity values"}
...
```{r fig.cap= "Boxplot of discount values. A strange looking box dataplot - median & 3rd quantile are the same (0.2) and not many orders have high discounts"}
...
```{r fig.cap = "Boxplot of profit. Most of the profits are outside of the box - but most of them clustered close to the box(not with so extreme values"}
boxplot(data$Profit)
```
...
```{r fig.cap= "Total sales per region."}
ggplot(data = data) + 
...
## Anomaly Detection using Local Outlier Factor Algorithm  -Nearest neighbour method 
...
Fonseca, Clustering Analysis in R using K-means. Retrieved from:  https://towardsdatascience.com/clustering-analysis-in-r-using-k-means-73eca4fb7967

Manimaran, Clustering Evaluation strategies. Retrieved from: https://towardsdatascience.com/clustering-evaluation-strategies-98a4006fcfc

Detect Anomalies in Time Series Using Anomalize Package In R. Retrieved from: https://www.analyticsvidhya.com/blog/2020/12/a-case-study-to-detect-anomalies-in-time-series-using-anomalize-package-in-r/
...
Filaire , Clustering on mixed type data. Retrieved from: https://towardsdatascience.com/clustering-on-mixed-type-data-8bbd0a2569c3

Mayer, Using OutForest. Retrieved from: https://cran.r-project.org/web/packages/outForest/vignettes/outRanger.html

Cheng-Chung Li, Detecting Outlier by Local Outlier Factor and Random Forest. Retrieved from: https://rstudio-pubs-static.s3.amazonaws.com/216270_ccd62999b6504c22a390bb39fee22f39.html
