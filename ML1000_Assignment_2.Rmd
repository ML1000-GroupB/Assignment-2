...
Using the Elbow method to determine the optimal number of clusters involves plotting the within cluster sum of squares. The optimal number of clusters is the one located at the "bend" on the graph, where WSS is lowered.
```{r}
set.seed(123)
...
```
The optimal number of clusters looks to be k = 4. 

Another method is to look at the silhouette method.
```{r}
fviz_nbclust(normal_transactions_scaled, kmeans, method = "silhouette", k.max = 24) + theme_minimal() + ggtitle("The Silhouette Plot")
```
The silhouette method suggests the optimal number of clusters is 2. 


Another method is using NbClust which provides 30 indices for determining the relevant number of clusters. It looks at all combinations of clusters, distance measures, and clustering methods. 

```{r}
# res.nbclust <- NbClust(normal_transactions_scaled, distance = "euclidean",
#                   min.nc = 2, max.nc = 9, 
...
Plotting the kmedoids with k =4
...
The average silhouette width of the total data set for k=5 is 0.08049662. 
