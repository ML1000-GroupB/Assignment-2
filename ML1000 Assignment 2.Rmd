...
#drop column Row ID because it is not necessary; it is the row number from the original excel file.
```{r}
data1["Row_ID"]<-NULL
```

The country variable is also not needed because all the values are United states:
```{r}
data1["Country"]<-NULL
```
...
```{r}
data1["Customer_Name"]<-NULL
```


Product ID and Product Name are also variables which give the same type of information. So we will drop product name.
```{r}
data1["Product_Name"]<-NULL
```

The Category column also is a bit too broad with just 3 categories of Office Supplies, Furniture, and Technology so we will also
drop this column.
```{r}
data1["Category"]<-NULL
```
